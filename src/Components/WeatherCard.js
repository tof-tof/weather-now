...
function WeatherCard({ place, onChecked }) {
  return (
    <div className="inline-block px-4 w-64 h-64 text-center"  key={place.name+place.country} onClick={()=>onChecked(place.name+place.country)}> 
      ...
      <img src={`http://openweathermap.org/img/wn/${place.icon}@2x.png`} alt={place.description+" icon"} className="w-full h-32" />
      ...
      <p className="text-xs text-right pt-2 italic ">last updated: { new Date().toLocaleString()}</p>
